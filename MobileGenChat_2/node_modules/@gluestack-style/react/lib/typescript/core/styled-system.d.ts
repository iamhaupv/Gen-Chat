export declare const CSSPropertiesMap: {
    alignContent: string;
    alignItems: string;
    alignSelf: string;
    aspectRatio: string;
    borderBottomWidth: string;
    borderLeftWidth: string;
    borderRightWidth: string;
    borderTopWidth: string;
    borderEndWidth: string;
    borderStartWidth: string;
    borderEndEndRadius: string;
    borderStartStartRadius: string;
    borderStartEndRadius: string;
    borderEndStartRadius: string;
    borderWidth: string;
    bottom: string;
    direction: string;
    display: string;
    flex: string;
    flexDirection: string;
    flexBasis: string;
    flexGrow: string;
    flexShrink: string;
    flexWrap: string;
    gap: string;
    rowGap: string;
    columnGap: string;
    height: string;
    justifyContent: string;
    left: string;
    margin: string;
    marginBottom: string;
    marginHorizontal: string;
    marginLeft: string;
    marginRight: string;
    marginTop: string;
    marginVertical: string;
    marginEnd: string;
    marginStart: string;
    maxHeight: string;
    maxWidth: string;
    minHeight: string;
    minWidth: string;
    padding: string;
    paddingBottom: string;
    paddingHorizontal: string;
    paddingLeft: string;
    paddingRight: string;
    paddingTop: string;
    paddingVertical: string;
    paddingEnd: string;
    paddingStart: string;
    position: string;
    right: string;
    top: string;
    width: string;
    zIndex: string;
    shadowColor: string;
    shadowOffset: string;
    shadowOpacity: string;
    shadowRadius: string;
    transform: string;
    transformOrigin: string;
    backfaceVisibility: string;
    backgroundColor: string;
    borderBottomLeftRadius: string;
    borderBottomRightRadius: string;
    borderColor: string;
    borderBlockColor: string;
    borderBlockEndColor: string;
    borderBlockStartColor: string;
    borderRadius: string;
    borderTopLeftRadius: string;
    borderTopRightRadius: string;
    opacity: string;
    overflow: string;
    overflowX: string;
    overflowY: string;
    borderBottomColor: string;
    borderLeftColor: string;
    borderRightColor: string;
    borderStyle: string;
    borderTopColor: string;
    elevation: string;
    color: string;
    fontFamily: string;
    fontSize: string;
    fontStyle: string;
    fontVariant: string;
    textTransform: string;
    fontWeight: string;
    lineHeight: string;
    textAlign: string;
    textDecorationLine: string;
    textShadowColor: string;
    textShadowOffset: string;
    textShadowRadius: string;
    textAlignVertical: string;
    verticalAlign: string;
    letterSpacing: string;
    textDecorationColor: string;
    textDecorationStyle: string;
    borderBottomEndRadius: string;
    borderBottomStartRadius: string;
    borderEndColor: string;
    borderStartColor: string;
    borderTopEndRadius: string;
    borderTopStartRadius: string;
    userSelect: string;
};
type reservedKeyTypes = 'isState' | 'isColorMode' | 'isPlatform' | 'isMediaQuery';
export type reservedKeyType = {
    key: string;
} & {
    [key in reservedKeyTypes]?: boolean;
};
export declare const reservedKeys: Record<string, reservedKeyType>;
export {};
//# sourceMappingURL=styled-system.d.ts.map