import { getComponentResolvedBaseStyle, getComponentResolvedVariantStyle, getDescendantResolvedBaseStyle, getDescendantResolvedVariantStyle } from './resolver/getComponentStyle';
...
export function updateOrderUnResolvedMap(theme, componentHash, declarationType, ExtendedConfig, _GluestackStyleSheet = GluestackStyleSheet, platform = '', isStyledComponent = false) {
  ...
  const orderedUnResolvedTheme = styledResolvedToOrderedSXResolved(unresolvedTheme);

  // platform is useful incase of Babel plugin
  if (platform === 'all') {
    INTERNAL_updateCSSStyleInOrderedResolvedWeb(orderedUnResolvedTheme, componentHash, true, prefixClassName, false);
  } else if (platform === 'web') {
    INTERNAL_updateCSSStyleInOrderedResolvedWeb(orderedUnResolvedTheme, componentHash, false, prefixClassName, false);
  } else {
    INTERNAL_updateCSSStyleInOrderedResolved(orderedUnResolvedTheme, componentHash, true, prefixClassName, false);
  }

  // base style
  const [componentOrderResolvedBaseStyle, componentOrderResolvedBaseStateStyle] = getComponentResolvedBaseStyle(orderedUnResolvedTheme);
  ...
  const [descendantOrderResolvedBaseStyle, descendantOrderResolvedBaseStateStyle] = getDescendantResolvedBaseStyle(orderedUnResolvedTheme);
  ...
  // variant style
  const [componentOrderResolvedVariantStyle, componentOrderResolvedVariantStateStyle] = getComponentResolvedVariantStyle(orderedUnResolvedTheme);
  ...
  const [descendantOrderResolvedVariantStyle, descendantOrderResolvedVariantStateStyle] = getDescendantResolvedVariantStyle(orderedUnResolvedTheme);
  ...
  const verbosedStyleIds = getStyleIds(orderedUnResolvedTheme, ExtendedConfig, shouldGuessDescendants);
  return {
