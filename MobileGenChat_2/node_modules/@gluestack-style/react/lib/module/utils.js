...
export function generateMergedThemeTokens(CONFIG) {
  const mergedTokens = CONFIG;
  const tokens = deepClone((CONFIG === null || CONFIG === void 0 ? void 0 : CONFIG.tokens) ?? {});
  const themeTokens = {};
...
export function convertToUnicodeString(inputString) {
  let result = '';
  if (!inputString) {
    return result;
  }
...
export function convertFromUnicodeString(inputString) {
  let result = '';
  if (!inputString) {
    return result;
  }
...
export function convertTokensToCssVariables(currentConfig) {
...
export const getCssVariableValue = (_object, _keyPath) => {
  // console.log(keyPath, 'key path here');
};
export function resolveAliasesFromConfig(config, props, ignoreKeys = new Set()) {
...
export function resolveStringToken(stringValue, config, tokenScaleMap, propName, scale, useResolvedValue = false, deleteIfTokenNotExist = false) {
...
export const getTokenFromConfig = (config, prop, value, useResolvedValue = false, deleteIfTokenNotExist = false) => {
...
export function getResolvedTokenValueFromConfig(config, _props, prop, value, useResolvedValue = false, deleteIfTokenNotExist = false) {
...
export function resolveTokensFromConfig(config, props, useResolvedValue = false, deleteIfTokenNotExist = false) {
  let newProps = {};
...
export function resolvedTokenization(props, config, ignoreKeys = new Set(), deleteIfTokenNotExist = false) {
...
export const shallowMerge = (target = {}, source) => {
  // console.setStartTimeStamp('deepMerge');
...
export const deepMergeArray = (target = {}, source) => {
  for (const key in source) {
    if (source.hasOwnProperty(key)) {
...
export function resolvePlatformTheme(theme, platform) {
