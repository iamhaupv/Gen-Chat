...
export function createMotionComponent(Component) {
  return /*#__PURE__*/forwardRef(function MotionComponent(_ref, // @ts-ignore
  ref) {
    let {
      animate,
      animateProps,
    ...
    const refAnims = useRef({}); // Generate the arrays of keys and values for transitioning. These are used as deps of useMemo
    ...
    const update = () => {
      ...
      for (let i = 0; i < animKeys.length; i++) {
        ...
        if (!anims[key] || anims[key].value !== value) {
          ...
          const isArr = isArray(valueInitial); // If this is the first run or it's a new key, create the Animated.Value
    ...
    const onLayout = transformOrigin ? useTransformOrigin(transformOrigin, style.transform, onLayoutProp) : onLayoutProp; // @ts-ignore
